...
const RARITIES = ["Common", "Uncommon", "Rare", "Very_Rare", "Visceral"];

interface CanvasProps {
  files?: { name: string; data: string }[];
  setCanvasURLs: React.Dispatch<
    React.SetStateAction<{ name: string; data: string; id: number }[]>
  >;
}
  ...
  useEffect(() => {
    if (files && canvas) {
      ...
      files.forEach((file) => {
        ...
        const matchingAddons = addonJson.filter((addon) =>
          addon.name.includes(file.name),
        );
  ...
  if (rarity) {
    addonBG =
      rarity === "common"
        ? addonBrownBG
        : rarity === "uncommon"
          ? addonGreenBG //uncommon is green
          : rarity === "rare"
            ? addonBlueBG //rare is blue
            : rarity === "very_rare"
              ? addonPurpleBG //very rare is purple
              : rarity === "ultra_rare"
                ? addonPinkBG //ultra rare is pink
                : addonEventBG;
    ...
    addonGrad =
      rarity === "common"
        ? addonBrownGrad
        : rarity === "uncommon"
          ? addonGreenGrad //uncommon is green
          : rarity === "rare"
            ? addonBlueGrad //rare is blue
            : rarity === "very_rare"
              ? addonPurpleGrad //very rare is purple
              : rarity === "ultra_rare"
                ? addonPinkGrad //ultra rare is pink
                : addonEventGrad;
  }
